...
`costs.sh` prints a node's incident link costs in topology-file order — exactly the order `bfproc`
...
## Build the node (`bfproc`)

```sh
make            # builds ./bfproc from bfproc.c + net_util.c (gnu11, -Wall -Wextra)
make clean      # removes bfproc + object files
```
(Plain build — `bfproc` uses no C11 threads, so no shim is needed; only `netproc` does.)

## Run a node

Form: `bfproc <netproc_addr> <node_id> <lifetime_sec> <cost1> [cost2 ...]`. Costs are positional, in
topology-file (= link index) order. For the given `topology.csv` (local, 60 s):

```sh
bfproc 127.0.0.1 56908 60 23 76          # links: 23->25824, 76->53021
bfproc 127.0.0.1 25824 60 23 62 213      # links: 23->56908, 62->7416, 213->15762
bfproc 127.0.0.1 7416  60 62 155 136     # links: 62->25824, 155->53021, 136->15762
bfproc 127.0.0.1 53021 60 155 76 79      # links: 155->7416, 76->56908, 79->15762
bfproc 127.0.0.1 15762 60 213 136 79     # links: 213->25824, 136->7416, 79->53021

# Or derive costs automatically:
bfproc 127.0.0.1 7416 60 $(sh build/costs.sh "source files/PA2_resource_alpha/topology.csv" 7416)
```
...
```sh
! sh build/step2_run.sh    # netproc + one bfproc (7416, 3s): connect, initial state, shutdown
```
...
Start `netproc`, then one `bfproc` per node; all converge to `Root=7416` (oracle in
...
```
Or manually: run `netproc` in one terminal and each `bfproc … ` (commands above) in others. Per-node
...
```sh
make clean
zip pa2_submission.zip README Makefile bf.h net_util.h net_util.c msg.h msg.c bfproc.c
```
