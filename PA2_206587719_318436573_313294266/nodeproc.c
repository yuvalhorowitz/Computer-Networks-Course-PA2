/*
 * nodeproc.c — the PA2 node process: main program, node state, and the
 * distributed Bellman-Ford logic.
 *
 * Purpose: one running nodeproc represents one node in the simulated network. It
...
#ifdef BF_DEBUG
				fprintf(stderr,
					"[nodeproc %u] recv on link %u from #%u: root=%u cost=%u exp=%u\n",
